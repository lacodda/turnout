...
```bash
turnout deploy [APP] [--server SERVER] [--no-build] [--clear]
```
...
turnout deploy myapp --clear            # clean the remote directory first
turnout deploy myapp --no-build         # upload what is already built
```

:::note
Remote backup and restore before/after deploy are on the [roadmap](https://github.com/lacodda/turnout#roadmap).
:::
