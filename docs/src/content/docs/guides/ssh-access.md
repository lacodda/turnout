...
Storing the passphrase in the keyring means turnout can use the key unattended, which is what a deploy needs. If you would rather the passphrase never be stored at all, hand the key to an [SSH agent](/turnout/concepts/ssh-agent/) instead and point the credential at it:

```bash
ssh-add ~/.ssh/id_ed25519_prod-deploy      # once per login session
turnout credential edit prod-deploy --auth agent
```

The agent then signs on the key's behalf, and turnout keeps no secret for that credential at all.
